...
public class Piece {
    ...
    static private int MAX_POINT = 4;
	/**
	...
	*/
	public Piece(TPoint[] points) {
        ...
        while (k < width) {
            for (int i = 0; i < body.length; ++i) {
                if (body[i].x == k && body[i].y <= skirt[k]) {
                    skirt[k] = body[i].y;
                    if (body[i].y < skirt[k])
                        k++;
                }
	...
	*/
	...
	public Piece computeNextRotation() {
        // CODE HERE
        TPoint[] nextBoby = new TPoint[this.body.length];
        for (int i = 0; i < body.length; ++i) {
            nextBoby[i] = new TPoint(body[i]);
        }
        for (int i = 0; i < nextBoby.length; ++i) {
            nextBoby[i].x = - body[i].y;
            nextBoby[i].y =   body[i].x;
        }
        int minHeight = nextBoby[0].y;
        int minWidth  = nextBoby[0].x;
        for (int i = 1; i < nextBoby.length; ++i) {
            if (minHeight > nextBoby[i].y)
                minHeight = nextBoby[i].y;
        }
        for (int i = 1; i < nextBoby.length; ++i) {
            if (minWidth > nextBoby[i].x)
                minWidth = nextBoby[i].x;
        }
        for (int i = 0; i < nextBoby.length; ++i) {
            if (minHeight < 0)
                nextBoby[i].y -= minHeight;
            if (minWidth <  0)
                nextBoby[i].x -= minWidth;
        }
        Piece nextRotation = new Piece(nextBoby);
        return nextRotation;
    }
